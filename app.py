...
app = Flask(__name__)
app.secret_key = "workpulse-secretariat-2025"
...
def seed_data():
    ...
    # Create sample tasks
    sample_tasks = [
        {'title': 'Design database schema',       'desc': 'Plan PostgreSQL schema for new project',          'pri': 'high',   'due': (today - timedelta(days=1)).strftime("%Y-%m-%d"), 'cat': 'Development',   'rec': 'none',   'emp': 0},
        {'title': 'Write unit tests',             'desc': 'Cover all API endpoints with pytest',              'pri': 'medium', 'due': today.strftime("%Y-%m-%d"),                      'cat': 'Testing',       'rec': 'none',   'emp': 0},
        {'title': 'Team standup meeting',         'desc': 'Daily sync with engineering team at 10am',         'pri': 'low',    'due': today.strftime("%Y-%m-%d"),                      'cat': 'Meetings',      'rec': 'daily',  'emp': 0},
        {'title': 'Fix API rate limiter',         'desc': 'Patch the rate limiter bug in production',         'pri': 'high',   'due': (today - timedelta(days=2)).strftime("%Y-%m-%d"), 'cat': 'Backend',       'rec': 'none',   'emp': 1},
        {'title': 'Write migration scripts',      'desc': 'DB migration for v2.1 schema changes',             'pri': 'medium', 'due': today.strftime("%Y-%m-%d"),                      'cat': 'Database',      'rec': 'none',   'emp': 1},
        {'title': 'Code review PRs',              'desc': 'Review open pull requests from teammates',         'pri': 'low',    'due': today.strftime("%Y-%m-%d"),                      'cat': 'Development',   'rec': 'daily',  'emp': 1},
        {'title': 'Unit test auth module',        'desc': 'Achieve 90% coverage on auth service',             'pri': 'high',   'due': (today + timedelta(days=1)).strftime("%Y-%m-%d"), 'cat': 'Testing',       'rec': 'none',   'emp': 2},
        {'title': 'Update Swagger docs',          'desc': 'Document all new v2 API endpoints',                'pri': 'medium', 'due': (today + timedelta(days=4)).strftime("%Y-%m-%d"), 'cat': 'Documentation', 'rec': 'none',   'emp': 2},
        {'title': 'Setup CI/CD pipeline',         'desc': 'Configure GitHub Actions for auto-deploy',         'pri': 'high',   'due': (today - timedelta(days=1)).strftime("%Y-%m-%d"), 'cat': 'DevOps',        'rec': 'none',   'emp': 3},
        ...
        {'title': 'Update SSL certificates',      'desc': 'Renew certs before they expire',                   'pri': 'high',   'due': (today + timedelta(days=5)).strftime("%Y-%m-%d"), 'cat': 'Security',      'rec': 'none',   'emp': 3},
    ]
...
def all_tasks():
    ...
    if emp_id:
        rows = Task.query.filter_by(assigned_to=emp_id).all()
    else:
        rows = Task.query.all()
    tasks = sorted([t.to_dict() for t in rows], key=lambda x: {'high': 0, 'medium': 1, 'low': 2}.get(x['priority'], 3))
    return jsonify({'tasks': tasks, 'stats': stats_for(tasks)})
